...
exports.playTurn = (req, res) => {
    ...
    try {
        ...
        if(result === false){
                                                                //сообщение о том, что не достаточно энергии, респонс который 
        }

        res.status(200).json({ message: 'Ход выполнен', match });//тут потом изменю на респонс, который изменяет параметры в игре
    } catch (error) {
